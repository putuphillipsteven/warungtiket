...
const { createTransactionQuery, findTransactionQUery } = require('../queries/transactionQuery');
...
const createTransactionService = async (
	...
	try {
		if (totalQuantity <= 0) throw new Error('Quantity cant be 0');
		if (referralUsed) {
		...
		const newReferral = await refferalCodeGenerator.alpha('uppercase', 5);
		...
		await createReferralQuery(newReferral, false, eventId, userId);
		return res;
	} catch (err) {
		throw err;
	}
