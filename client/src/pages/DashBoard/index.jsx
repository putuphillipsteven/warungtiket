...
export default function DashBoard(props) {
	...
	const selectedUser = user.find((user) => user.id === +loggedInUser.id);

	const buyedEvent = () => {
		let newArr = [];
		selectedUser?.transactions?.map((transaction) => newArr.push(transaction));
		return newArr;
	};
	const events = useSelector((state) => state?.events?.events);

	const buyedEvents = buyedEvent();
	const filteredEvents = events.filter((el) => buyedEvents.includes(el.userId));
	return (
		<Box>
			<Navbar display={'none'} />
			<Box p={'0 3.5em'} mt={'10em'} mb={'2em'} minHeight={'100vh'}>
				<Box display={'flex'}>
					...
					<Box w={'full'}>
						<VStack align={'stretch'}>
							<Profile email={selectedUser?.email} username={selectedUser?.username} />
