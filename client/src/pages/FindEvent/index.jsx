...
function FindEvent() {
	...
	const fetchEvent = async () => {
		try {
			const res = await axios.get(`http://localhost:8000/event?provinceId=${locationFilter}`);
	...
	return (
		<Box>
			...
			/>
			<Box p={'0 3.5em'} mt={'10em'} mb={'2em'} display={'flex'} minHeight={'100vh'}>
				...
				<Box w={'full'} pt={'0 3.5em'}>
					...
					<Box mt={'.5em'} ml={'.5em'}>
						<Grid templateColumns={'repeat(4, 1fr)'} templateRows={'repeat(3, 1fr)'} gap={'.5em'}>
							{renderedEvents}
						</Grid>
					</Box>
				</Box>
